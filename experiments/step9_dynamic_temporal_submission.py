"""
Step 9 — Dynamic Temporal IoU Submission
Restored to the proven v2 prediction baseline.

The earlier external rank-blend path degraded the documented v2 score,
so this script now uses models/test_preds_v2.npy directly and only applies
dynamic temporal-window estimation.

Inputs:  models/test_preds_v2.npy
...
if daily_parts:
    ...
    for acc_id in predicted_mule_ids:
        ...
        # Create a complete date range to handle gaps (fill with 0)
        if len(dates) < 2:
            # Single day of activity — use that day
            windows[acc_id] = (
        ...
        # Baseline = median of daily volume (including zeros for inactive days)
...
if windows:
    ...
    for acc_id, (s, e) in windows.items():
        dt_start = pd.Timestamp(s)
        dt_end = pd.Timestamp(e)
        window_days.append((dt_end - dt_start).days)

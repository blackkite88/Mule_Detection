"""
Step 6v3 — LightGBM + XGBoost Only (No CatBoost)
Replicates the proven v2 approach minus CatBoost, with verbose training.

- LightGBM + XGBoost with 5-fold stratified CV
- Optimized blend weights (Nelder-Mead)
- Rank + probability blend comparison
- F1 threshold optimization
- Dynamic temporal windows
- Generates submission.csv

Shows full training progress for monitoring.
"""
import polars as pl
import numpy as np
...
for c in non_numeric:
    print(f"  WARNING: dropping non-numeric '{c}' (dtype={train[c].dtype})")
    FEATURES.remove(c)
...
# ═══════════════════════════════════════════════════════════════════════
print("\n" + "=" * 70)
print("Training LightGBM + XGBoost (5-fold CV)")
print("=" * 70)
...
for fold, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train)):
    ...
    print(f"\n{'─'*60}")
    print(f"FOLD {fold} | train={len(train_idx)} val={len(val_idx)} (mule rate: {yval.mean():.4f})")
    print(f"{'─'*60}")
...
res = minimize(neg_auc, x0=[0.5, 0.5], method="Nelder-Mead", options={"maxiter": 1000})
w = np.abs(res.x)
w = np.clip(w, 0.1, None)
w = w / w.sum()
...
try:
    ...
    v2_auc = roc_auc_score(y_train, v2_oof)
    print(f"  ★ v2 reference OOF AUC: {v2_auc:.5f}")
    diff = best_auc - v2_auc
    print(f"    Δ = {diff:+.5f} ({'BETTER' if diff > 0 else 'WORSE' if diff < 0 else 'SAME'})")
except Exception:
    pass
...
np.save(MODELS / "oof_lgb_v3.npy", oof_lgb)
np.save(MODELS / "oof_xgb_v3.npy", oof_xgb)
...
test.select("account_id").write_parquet(MODELS / "test_ids_order_v3.parquet")
...
print(f"\n✓ All v3 artifacts saved")

# ═══════════════════════════════════════════════════════════════════════
# DYNAMIC TEMPORAL WINDOWS + SUBMISSION
# ═══════════════════════════════════════════════════════════════════════
print("\n" + "=" * 70)

"""
Step 9v8 — Final V8 Submission (Refined Ensemble + F1 Prior + Lifecycle IoU)

This script implements:
1. Refined Ensemble: Uses rank-blended predictions from V8 (SHAP-pruned subset).
...
Inputs:  models/test_preds_v8.npy (LGB+XGB+CAT Rank Blend on Pruned Features)
...
# 1. Load V8 Ensemble Predictions
# ─────────────────────────────────────────────────────────────────────────────
test_ids = pl.read_parquet(MODELS / "test_ids_order.parquet")
preds = np.load(MODELS / "test_preds_v8.npy")
...
print("\nTop predictions (V8 Ensemble Ranks):")

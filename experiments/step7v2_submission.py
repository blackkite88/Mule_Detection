"""
Step 7v2 — Submission with V2 Predictions + Dynamic Temporal Windows
Uses the proven v2 prediction set saved by step6v2_model_training.

This restores the last strong baseline by avoiding the later external
rank-blend path and only improving the temporal-window estimation.
...
preds = np.load(MODELS / "test_preds_v2.npy")
...
if windows:
    window_days = []
    for acc_id, (start, end) in windows.items():
